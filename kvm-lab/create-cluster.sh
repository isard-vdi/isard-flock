...
for ((i=1; i<=$1; i++)); do
	...
	if [[ $i == 1 ]]; then
		...
		command="ExecStart=/bin/bash -c 'cd /opt/isard-flock/ \&\& ./install-isard-flock.sh --master 1 --if_viewers eth0 --if_nas eth1 --if_drbd eth2 --raid_level 1 --raid_devices /dev/vdb,/dev/vdc --pv_device /dev/md0 --isard_volume_size 470M --espurna_apikey 0123456789ABCDEF 1>/tmp/auto-install.log 2>/tmp/auto-install-error.log'"
