...
for ((i=1; i<=$1; i++)); do
	...
	if [[ $i == 1 ]]; then
		...
		qemu-img create -f qcow2 /var/lib/libvirt/images/if$i-d1.qcow2 4G
		qemu-img create -f qcow2 /var/lib/libvirt/images/if$i-d2.qcow2 4G
		...
		virt-copy-in -d if$i ./auto-install.service /opt/
	fi
	if [[ $i == 2 ]] || [[ $i == 3 ]]; then
		qemu-img create -b /var/lib/libvirt/images/centos7.qcow2 -f qcow2 /var/lib/libvirt/images/if$i.qcow2
		qemu-img create -f qcow2 /var/lib/libvirt/images/if$i-d1.qcow2 4G
